/*package it.unibo.bd.db.controller.show;
...
import it.unibo.bd.db.DBConnection;
import it.unibo.bd.db.controller.ControllerMethods;
import it.unibo.bd.db.controller.InserimentoDatiTorneoController;
import it.unibo.bd.db.controller.VIEW;
import it.unibo.bd.db.models.ScacchistaModel;
...
public class ShowPartecipantiTorneoController extends ControllerMethods{

	InserimentoDatiTorneoController datiTorneo = new InserimentoDatiTorneoController();
    @FXML
    ...
    void showOnMouseClicked(MouseEvent event) throws SQLException {
    	...
    	/*ResultSet rs = statement.executeQuery("SELECT * FROM scacchisti as s, prenotazioni as p "
    			+ "WHERE p.CF_Scacchista = s.CF "
    			//+ "AND DataTorneo = '" + datiTorneo.getDataTorneo() + "';" );
    	while(rs.next()) {
